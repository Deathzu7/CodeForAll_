...
public class LinkedList<T> implements Iterable<T> {
    ...
    public <R> LinkedList<R> map(Function<T, R> mapper) {
        ...
        LinkedList<R> linkedListMap = new LinkedList<>(); // Criação de uma nova linkedList
        for (T element : this) { //  Vamos percurrer toda a lista
            R mapperMap = mapper.apply(element); //
            linkedListMap.add(mapperMap); // Adicionar os valor transformados na lista
        }
        return linkedListMap; // Vai retornar a nova lista.
    ...
    public LinkedList<T> filter(Predicate<T> predicate) {
        // Temos com a lista em cima, criar uma nova lista com o filtro que queremos (como numeros pares).
        LinkedList<T> linkedListFilter = new LinkedList<>(); // Crianmos uma nova lista
        for (T element : this) { // Percurrer todos os elementos da lista
            if (predicate.test(element)) { // Dependendo do filtro, adicionar os elementos que pertencem ao filtro à lista.
                linkedListFilter.add(element);
            }
        }
        return linkedListFilter; // retorna uma nova lista com os elementos selecionados pelo filtro.
    }
    ...
    public T reduce(BinaryOperator<T> reducer, T accumulator) {
        for (T element : this){ //Percorre todos elementos da lista
         accumulator = reducer.apply(accumulator, element); // Aplica a função reduce
        }
        return accumulator; // retorna o valor final do accumulator
    }
    ...
    public boolean anyMatch(Predicate<T> predicate) {
        for (T element : this){ //Percorre todos elementos da lista
         if(predicate.test(element)){ //Verifica se o elemento satisfaz a condiçao do predicate
             return true; //Retorna true se algum elemento corresponder
         }
        }

       return false; //Retorna false se não retornar nenhum elemento
    }
    ...
    public boolean allMatch(Predicate<T> predicate) {
        for(T element : this){ //Percorre todos elementos da lista
            if(!predicate.test(element)){ //Verifica se o elemento não satisfaz a condição do predicate
                return false;
            }
        }
